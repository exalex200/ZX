...
elseif tonumber(user_id) == tonumber(0000) then
var = true  
elseif tonumber(user_id) == tonumber(1819600748666555555555) then
var = true  
elseif tonumber(user_id) == tonumber(0000) then
var = true  
elseif tonumber(user_id) == tonumber(0000) then
...
send(msg.chat_id_, msg.id_,' ✶ اصدار سورس VIP{ S:12✓}')
end
if text == '• ━━━━━━❪الركن الاخير❫━━━━━━ •' and DevSoFi(msg) then
local Text = [[ 
[قناه سورس VIP ادخل وتابع الجديد](t.me/UUK88)
]] 
keyboard = {}  
keyboard.inline_keyboard = { 
{{text = '00:00', url="t.me/UUK88"}}, 
...
database:del(bot_id..'Srt:Bot') 
local Text = [[ 
 ✶ من أحسن السورسات على التليجرام سورس VIP ✶
...
if tonumber(result.sender_user_id_) == tonumber(1819600748) then  
send(msg.chat_id_, msg.id_, " ✶ لا تسطيع حظر مبرمج السورس عام")
return false 
end
...
if result.id_ == tonumber(1819600748) then
send(msg.chat_id_, msg.id_, " ✶ لا يمكنك حظر مبرمج السورس \n")
return false 
end
...
if tonumber(userid) == tonumber(1819600748) then  
send(msg.chat_id_, msg.id_, " ✶ لا تسطيع حظر مبرمج السورس عام")
return false 
end
...
------------------------------------------------------------------------ adddev2 sudog
if text == ("رفع VIP") and tonumber(msg.reply_to_message_id_) ~= 0 and SudoBot(msg) then
...
if text and text:match("^رفع VIP @(.*)$") and SudoBot(msg) then
local username = text:match("^رفع VIP @(.*)$")
function Function_DRAGON(extra, result, success)
if result.id_ then
if (result and result.type_ and result.type_.ID == "ChannelChatInfo") then
send(msg.chat_id_,msg.id_,"✶︙ عذرا عزيزي المستخدم هاذا معرف قناة يرجى استخدام الامر بصوره صحيحه !")   
...
if text and text:match("^رفع VIP (%d+)$") and SudoBot(msg) then
local userid = text:match("^رفع VIP (%d+)$")
...
local Name_Bot = (database:get(bot_id..'Name:Bot') or 'سVIP')
...
if text == 'العاب VIP ' or text == 'الالعاب المتطوره' or text == 'العاب متطوره' then  
local Text = [[  
